...
def getSurround(x, y, maze_state):
    surrounding_cells = []
...
def isAccessible(x, y, wall_position, maze_state):
    # get surrounding cells
    surrounding_cells, _ = getSurround(x, y, maze_state)
...
# set goal
def setGoal(maze_state, preset='center', cx=0, cy=0):
...
def backtrace_dfs(x, y, orientation, wall_position, lifo_stack, trace_stack,
                  maze_state, history):
    ...
    while (True):
        ...
        accessible_cells = isAccessible(x, y, wall_position, maze_state)
        ...
        if (len(accessible_cells) == 0):
            ...
            # showMaze(next_cell[0], next_cell[1], orientation, history,
...
def next_move_dfs(x, y, orientation, wall_position, lifo_stack, trace_stack,
    ...
    visited_cell = [
        cell for cell in accessible_cells if maze_state[cell[1]][cell[0]] == 1
    ]
    ...
    if (len(accessible_cells) != 0):
        ...
        orientation, x, y = backtrace_dfs(x, y, orientation, wall_position,
                                          lifo_stack, trace_stack, maze_state,
                                          history)

        return next_move_dfs(x, y, orientation, wall_position, lifo_stack,
...
def main():
    ...
    while (True):
        ...
        if (check_goal_dfs(X, Y, goal)):
            ...
            # elif (state == 1):
            #     # to center
            #     maze_state = setGoal(maze_state, 'center')
            # elif (state == 2):
            #     # to top right corner
            #     maze_state = setGoal(maze_state, 'topright')
            # elif (state == 3):
            #     # to center
            #     maze_state = setGoal(maze_state, 'center')
            # elif (state == 4):
            #     # to top left corner
            #     maze_state = setGoal(maze_state, 'topleft')
            # elif (state == 5):
            #     # to starting position, bottom left
            #     maze_state = setGoal(maze_state, 'bottomleft')
            # elif (state == 6):
            ...
            show(maze_state)
            # shortest_path = predictPath(wall_position, maze_state, shortest_path,
            #                             history)

            orientation, X, Y = next_move_dfs(X, Y, orientation, wall_position,
                                              lifo_stack, trace_stack,
                                              maze_state, history,
                                              cell_parent_map)
